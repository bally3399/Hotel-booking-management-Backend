...
//            RoomRequest roomRequestDto = objectMapper.readValue(roomJson, RoomRequest.class);
//
...
//            RoomResponse response = roomServiceImpl.createRoom(roomRequestDto, pictures);
//
...
//    @PreAuthorize("hasRole('ROLE_ADMIN')")
//    @PutMapping("/{id}")
//    public ResponseEntity<?> editRoomById(@PathVariable Long id, @RequestBody RoomRequest roomRequestDto) {
//        try {
//            if (id == null || id <= 0) {
//                return ResponseEntity.badRequest()
...
//            }
//
//            List<RoomResponse> filteredRooms = roomServiceImpl.filterHotelRoomByType(hotelId, type);
...
//            }
//
//            List<RoomResponse> filteredRooms = roomServiceImpl.filterByPriceAndState(minPrice, maxPrice, state);
//            if (filteredRooms.isEmpty()) {
//                return ResponseEntity.status(HttpStatus.NOT_FOUND)
